...
describe("List component", () => {
    ...
    it("renders a remove button for each generated component and one add button at end of list", () => {
        ...
        const componentElements = [...screen.getByRole('generic', {name: 'Education'}).children[0].children]
      ...
      it('renders new component section when add button clicked', () => {
        render(<List label="Education" place="schools"/>)
        expect(screen.getByRole('generic', {name: 'Education'}).children.length).toBe(1)
        userEvent.click(screen.getByRole('button', {name: 'Add'}))
        expect(screen.getByRole('generic', {name: 'Education'}).children.length).toBe(2) 
      })
      ...
      it('removes component section when remove button clicked', () => {
        ...
        expect(screen.getByRole('generic', {name: 'Education'}).children.length).toBe(0) 
      })
  });